...
class ProductProduct(osv.osv):
    ...
    def create_valuation(self, cr, uid, ids, context=None):
        ...
        @param context: 'product_uom': UoM conversion will be attempted.
                        'date': the quantity will be queried at that date-time and the valuation will be recorded at that date-time.
                            WARNING! the price CANNOT be queried at that date, so it may be inconsistent.
                        'name': all created records will have that name instead of the name of the product
                        Other keys may be added to tweak the way quantities are queried (shop, warehouse...)
        ...
        for product in self.browse(cr, uid, ids, context=context):
            ...
            valuation_ids.append(val_obj.create(cr, uid, default, context=context))
